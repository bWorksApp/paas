...
const PublishContract = (props) => {
  ...
  const { publishedContracts = 0, approvedContracts = 0 } = props;
  ...
  return (
    <CardWithIcon
      ...
      subtitle={`${publishedContracts} published contracts, ${approvedContracts} approved contracts`}
      minHeight={890}
    >
      <List sx={{ display }}>
        {postedjobs?.map((record: any) => (
          <>
            ...
            <ListItem
              ...
              /*    button
              component={Link}
              to={`/jobapprovedContracts/?filter=${JSON.stringify({ jobId: record.id })}`} */
